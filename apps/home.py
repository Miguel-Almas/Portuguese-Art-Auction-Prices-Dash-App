...
df = pd.read_csv(path+"data_cml_processed.csv",index_col=0)

model_xgb = xgboost.XGBRegressor()
model_xgb.load_model(path+'model.bin')
joblib_model = joblib.load(path+'ridge_model.pkl')
...
layout = html.Div([
    dbc.Container([
        dbc.Row([
          dbc.Col(
            image_filename = path+"hammer.png" # replace with your own image
            encoded_image = base64.b64encode(open(image_filename, 'rb').read())
            html.Img(src='data:image/png;base64,{}'.format(encoded_image),style={'width':'500px','height':'320px'}),style={'margin-top':'35px'}
...
def get_predictions(dim_1,dim_2,dim_3,birth_date,death_date,artist,technique):
  ...
  #Mean Encodings
  df_map_author = pd.read_csv(path+'author_mapping_mean_encodings.csv')
  df_map_colours = pd.read_csv(path+'colours_mapping_mean_encodings.csv')
